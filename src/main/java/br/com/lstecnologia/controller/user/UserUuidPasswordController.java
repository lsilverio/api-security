...
public class UserUuidPasswordController extends BaseController {
	...
	@ApiOperation(value = "Generate uuid for password update and send by email", response = Void.class)
	public ResponseEntity<Void> generateCode(@Valid @RequestBody PasswordUserRequestDTO passwordUserRequestDTO) {
		userUuidPasswordService.generateCode(passwordUserRequestDTO);
		return noContent();
	}
